...
软件的本质就三件事：**计算、存储、展示**。任何一个完整的软件产品都在做这三件事——拿到数据（存储），处理数据（计算），把结果呈现给人（展示）。

现在的 Skill（SKILL.md + 脚本）只约束了**计算过程**——告诉 agent 怎么调 API、怎么处理逻辑。但存储和展示几乎是空白：

| 能力 | 传统软件 | 当前 Skill |
| --- | --- | --- |
| **计算** | 后端逻辑 | ✅ SKILL.md + 脚本（唯一成熟的部分） |
| **存储** | 数据库 | ❌ 没有标准方案，靠临时文件或外部服务 |
| **展示** | 前端 UI | ❌ 只能输出纯文本流 |

**Skill 本质上是一个只有后端、没有前端和数据库的软件。** 这就像 2000 年代的 CGI 脚本——能算，但没有持久化，也没有像样的界面。
